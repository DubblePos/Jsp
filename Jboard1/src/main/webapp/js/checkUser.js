...
var reUid = /^[a-z]+[a-z0-9]{3,10}$/; //a-z 되고 영문과 숫자가 조합 최소4자리에서 최대10자리가 올수있다.
var rePass = /^(?=.*[a-zA-Z])(?=.*[0-9]).{4,}$/;//{3,10}안에 숫자 띄어쓰기하면 안됨!!!엄격!!
var reName = /^[가-힣]{2,10}$/;
    				...
    				success: function(data){
    					...
    					if(data.result ==1){
							...
							if(reUid.test(uid)){
								...
								$('.resultId').css('color', 'red').text('아이디는 영문 소문자 또는 숫자 조합 4~10자 까지 입니다.');
								isUidOk = false;
							}
						}
    				}
    				...
    				success: function(data){
    					...
    					if(data.result ==1){
							...
							if(reNick.test(nick)){
								...
								$('.resultNick').css('color','red').text('닉네임은 영문,한글,숫자포함 2~10입니다.');
								isNickOk = false;
							}
       					}
    				}
